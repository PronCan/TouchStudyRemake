...
public class MistakeQuestionsList extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.list_mistakes);

        mAdapter = new ArrayAdapter<String>(getApplicationContext(), R.layout.list_questions_detail, R.id.tv_listQuestion);
        mistakesList = (ListView) findViewById(R.id.listView_mistakesList);
        mistakesList.setAdapter(mAdapter);
        mistakesList.setOnItemClickListener(onItemClickListener);

        mDBHelper = new DBHelper(this);


        try {
            questions = mDBHelper.getMistakeQuestions();
            for(int i=0; i<questions.size(); i++) {
                mAdapter.add(questions.get(i).question);
            }
        } catch (Exception e) {
            Log.d("%%%TAG_Mistake%%%", e.getMessage().toString());
        }
    }
